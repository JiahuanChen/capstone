...
#	Read Me 	#
#################
# Command Example : $ python str_similar.py --infile b.txt
# Choose a txt file as an input 
import argparse
import os
...
# parser = argparse.ArgumentParser(description="Divied into mulptiple txt files from a file according to string similarity")
# parser.add_argument('--infile', dest='infile', help='Specify the filename as input')
...
# 		assignee_list.append(assignee_name[i])
# 		group_number += 1
# 		mypath = os.getcwd() + '/' + args.infile[0] + '/'
# 		if not os.path.isdir(mypath):
# 			os.makedirs(mypath)
# 		with open(mypath + args.infile[0]+str(group_number)+'.txt','w') as fw:
# 			fw.write(patent_assignee[i])
# 			for j in range(i+1,len(assignee_name)):
# 				assignee1 = str.split(assignee_name[i],'+')[0]
# 				assignee2 = str.split(assignee_name[j],'+')[0]
